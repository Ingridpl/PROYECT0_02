...
def extraccion_rutas(direccion, nombre_archivo_cuenta, nombre_archivo_ingreso):
    ...
    RUTAS = []
    n = 0   #los siguientes for solo filtran las primeras 10 rutas
    for ruta in conteo_rutas:
        if n <10:
            RUTAS.append(ruta)
            n += 1
    ...
    for ruta in conteo_rutas:
        if n <10:
            rutas_poringreso.append(ruta)
            n += 1
    escritor_csv(nombre_archivo_cuenta, RUTAS)
    escritor_csv(nombre_archivo_ingreso, rutas_poringreso)
...
#asi podremos ver que paises generan el 80% de ingresos
suma_porcentaje = 0
